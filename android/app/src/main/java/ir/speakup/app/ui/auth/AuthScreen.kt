...
fun AuthScreen(onDone: () -> Unit, vm: AuthViewModel = hiltViewModel()) {
    ...
    Column(
        ...
        if (s.step == AuthStep.PHONE) {
            ...
            // «بیش از ۲۰۰۰» و نه یک عدد دقیق.
            //
            // اینجا هنوز محتوا وارد دیتابیس نشده، پس نمی‌شود مثل صفحه
            // پرداخت عدد را از دیتابیس خواند. عدد دقیقِ دستی هم با اولین
            // تغییر محتوا کهنه می‌شود و کسی یادش نمی‌ماند درستش کند —
            // اینجا «۲۴۰۰» نوشته بود در حالی که واقعیت ۲٬۰۸۸ است.
            //
            // اسکریپت خروجی محتوا این کف را می‌سنجد و اگر شمار واقعی
            // واژه‌ها پایین‌تر بیاید، خطا می‌دهد.
            Promise("📚", "بیش از ۲۰۰۰ واژه", "از مقدماتی تا متوسط، با تصویر و صدا")
